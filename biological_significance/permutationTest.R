...
canonical = c(3, 6, 3, 4, 4, 2, 8)
nonCanonical = c(0, 1, 3, 0)
...
ggplot() +
  stat_bin(data = diffsTibble,
                 mapping = aes(x = value), 
                 stat="count", bins=20) +
  stat_bin(data = extremeDiffs, mapping = aes(x = value, fill = "more extreme than actual difference"), stat="count", bins=20) +
